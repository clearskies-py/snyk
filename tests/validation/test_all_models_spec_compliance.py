...
MODEL_DESTINATION_MAP = {
    ...
    "SnykContainerImageTargetRef": (
        "orgs/{org_id}/container_images/{id}/relationships/image_target_refs",
        SnykContainerImageTargetRef,
    ),
...
class TestAllModelsHaveDestinationName:
    ...
    @pytest.mark.parametrize(
        "model_name,expected_pattern,model_class",
    ...
    def test_model_destination_name_matches_pattern(
        ...
        actual = model_class.destination_name()  # type: ignore[attr-defined]
...
class TestAllModelsHaveIdColumn:
    ...
    @pytest.mark.parametrize(
        ...
        [(name, cls) for name, (_, cls) in MODEL_DESTINATION_MAP.items() if name not in MODELS_WITHOUT_ID_COLUMN],
    )
    ...
    @pytest.mark.parametrize(
        "model_name,model_class",
        [(name, cls) for name, (_, cls) in MODEL_DESTINATION_MAP.items() if name in MODELS_WITHOUT_ID_COLUMN],
    )
...
class TestModelEndpointsExistInSpec:
    ...
    def test_model_endpoint_exists_in_spec(
        self, spec_paths: set[str], model_name: str, expected_pattern: str, model_class: type
    ) -> None:
        ...
        path_exists = (
            normalized in spec_paths
            or normalized.lstrip("/") in spec_paths
            or any(normalized in p or p.endswith(normalized) for p in spec_paths)
        )
        ...
        if not path_exists:
            ...
            path_exists = any(p.startswith(normalized) or p.startswith(normalized.lstrip("/")) for p in spec_paths)
    ...
    @pytest.mark.parametrize(
        "model_name,expected_pattern,model_class",
        [(name, pattern, cls) for name, (pattern, cls) in MODEL_DESTINATION_MAP.items() if name in V1_API_MODELS],
    )
    def test_v1_api_model_uses_v1_path_format(self, model_name: str, expected_pattern: str, model_class: type) -> None:
...
class TestSpecCoverageByTag:
    ...
    def test_orgs_tag_coverage(self, spec: dict[str, Any]) -> None:
        ...
        orgs_models = [SnykOrg, SnykOrgMember, SnykOrgMembership, SnykOrgInvite, SnykOrgUser]
    ...
    def test_projects_tag_coverage(self, spec: dict[str, Any]) -> None:
        ...
        projects_models = [SnykProject, SnykProjectHistory, SnykProjectIgnore, SnykProjectSbom]
    ...
    def test_settings_tag_coverage(self, spec: dict[str, Any]) -> None:
        ...
        settings_models = [SnykOrgSettingsIac, SnykOrgSettingsSast, SnykOrgSettingsOpenSource, SnykGroupSettingsIac]
