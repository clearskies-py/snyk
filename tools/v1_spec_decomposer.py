#!/usr/bin/env python3
"""
Decomposes the Snyk V1 API spec (YAML) into manageable chunks for LLM processing.

This script splits the Snyk V1 API specification into smaller,
domain-specific chunks that can be processed within LLM context windows.

Usage:
    python tools/v1_spec_decomposer.py [--spec PATH] [--output DIR]

Example:
    python tools/v1_spec_decomposer.py --spec api_spec/v1-api-spec.yaml --output api_spec/v1_decomposed
"""
...
COMMON_SCHEMA_NAMES = {
    # Notification settings (shared across orgs and users)
...
def decompose_spec(
    ...
    Args:
        spec_path: Path to the OpenAPI spec YAML file
...
def print_summary(manifest: dict[str, Any]) -> None:
    ...
    print("\n" + "=" * 60)
    print("V1 SPEC DECOMPOSITION SUMMARY")
    print("=" * 60)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the OpenAPI spec YAML file",
    )
    parser.add_argument(
        "--output",
        default="api_spec/v1_decomposed",
    ...
    print(f"Loading V1 spec from: {args.spec}")
