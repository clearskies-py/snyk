...
def extract_models(models_dir: str | Path = "src/clearskies_snyk/models") -> dict[str, ModelInfo]:
    ...
    for model_file in models_dir.glob("*.py"):
        ...
        # Extract columns
        columns = []
        column_pattern = r'^\s+(\w+)\s*=\s*(String|Boolean|Integer|Datetime|Json|Select|HasMany|BelongsTo|ProjectTagList)\s*\('
...
def extract_endpoints(spec: dict[str, Any]) -> dict[str, list[EndpointInfo]]:
    ...
    for path, methods in spec.get("paths", {}).items():
        ...
        for method, details in methods.items():
            ...
            endpoints[normalized].append(EndpointInfo(
                path=path,
                method=method.upper(),
    ...
    for col in searchable_columns:
        if col not in query_params:
            issues.append(ComplianceIssue(
                ...
                details=f"Available query params: {', '.join(sorted(query_params)[:10])}...",
            ))
...
def run_compliance_check(
    spec_path: str,
    models_dir: str = "src/clearskies_snyk/models",
    verbose: bool = False
) -> list[ComplianceIssue]:
...
def print_report(issues: list[ComplianceIssue]) -> None:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Check model compliance with OpenAPI spec"
    )
    parser.add_argument(
        "--spec",
